...
Кэш память в процессорах разделяется на три уровня. С возрастанием уровня объем кэша возрастает, а время доступа к нему увеличивается. Время доступа к любому из
кэшей значительно меньше времени доступа к оперативной памяти (за счет расположения вблизи процессора). При этом ограничением является объем кэшей, так как именно из-за малого размера они позволяют добиться высокой скорости.
...
В кэш данные попадают при первом обращении (причем изначально данные помещаются в кэши высших порядков, а при последующих обращениях переехжают в более близкие к процессору кэши). В дальнейшем данные могут быть замещены другими. Однако в 
таком случае мы рискуем часто промахиваться мимо кэша, так как в некоторых ситуациях данные будут вытеснены из кэша до следующего обращения к ним. 
...
В случае многопоточного исполнения кэши могут породить некоторые проблемы (несогласованность кэшей). Проиллюстрировать данную проблему можно следующим
примером:
...
* Возможность реализовать параллелизм по задачам (несвязанные участки кода) или по данным (например циклы)
...
* Инструментирование может быть выполнено на более высоком уровне. Инструмент Coccinelle добавляет в исходный код инструкции, позволяющие отслеживать и профилировать исполнение
* Профилирование сэмплами - можем считать различные события (такты процессора, промахи кэшей, ошибки предсказания переходов, etc). Пример - утилита `perf`. Это происходит с помощью аппаратных счетчиков
